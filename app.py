...
# --- タイトル表示（元のst.titleからデザイン統一のヘッダーへ変更） ---
st.markdown("""
...
# ==================================================
    ...
    if st.button("🔄 ちょっとアドバイスが自分らしくない（別の視点で聞き直す）"):
        genai.configure(api_key=api_key)
        model = genai.GenerativeModel('gemini-2.5-flash')
        reset_prompt = "求職者から「先ほどのアドバイスは自分らしくない」とフィードバックがありました。温かく受け止め、全く別の角度（素直な気持ちを聞くなど）から、やさしい質問を1つだけ投げかけてください。全文の代筆は厳禁。"
...
# ==================================================
    ...
    # ダウンロードボタン
    st.download_button(label="📝 完成した志望動機を保存（ダウンロード）する", data=final_text, file_name="my_motivation_letter.txt", mime="text/plain")
